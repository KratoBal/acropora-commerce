 ...
 * for what that costs.
 *
 * IT WRITES NO `shipping_payment_rule` ROWS. That table is EMPTY on the test
 * machine too, so there is nothing to copy across - the pairings are a separate
 * decision, not a value someone forgot to type.
 *
        ...
        regions: [
          {
            ...
            payment_providers: ["pp_system_default", "pp_acropora_cod"],
          },
        ],
      },
    })
    region = result[0]
    logger.info(`Régió létrehozva: ${REGIO_NEVE} (huf)`)
  }
      ...
      input: [
        {
          ...
          default_tax_rate: { name: "Áfa", rate: 27 },
        },
      ],
    })
    logger.info("Adóterület létrehozva: hu, 27 százalékos alapkulccsal.")
  }
