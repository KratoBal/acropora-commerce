 ...
 *
 * Every input is an explicit product flag. Heavy goods in particular are NOT
 * derived from `ProductVariant.weight`: Acropora keeps no reliable shipping
 * weights, so the product is marked by hand, exactly as in the UNAS webshop
 * this replaces.
 *
...
export type ShippingRelevantItem = {
  ...
  /**
   * Heavy goods, set by hand on the product.
   *
   * NOT derived from weight. Acropora does not maintain reliable product
   * weights for shipping decisions, and the UNAS webshop this replaces is
   * operated by marking products manually. That operational model is kept
   * deliberately, so no weight threshold exists here.
   */
...
/** An unset flag is not a restriction, so only an explicit true counts. */
const isHeavy = (item: ShippingRelevantItem) => item.is_heavy === true
 ...
 * The four classes are mutually exclusive by construction, and every input is
 * an explicit flag: nothing is inferred from weight, names or categories.
 */
